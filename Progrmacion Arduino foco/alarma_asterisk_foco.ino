...
const char* serverNameBD = "http://192.168.7.3/Alarma_asterisk/insertar_datos.php";
const char* serverNameFlask = "http://192.168.7.2:80/datos";
...
const int ledLuzBajo = 14;   // PIN D5 LED para indicar luz baja
...
void loop() {
  ...
  float luminosity = map(fotoresistorValue, 0, 1023, 0, 100);
    ...
    if (luminosity < 20) {
      ...
      digitalWrite(ledLuzMedio, LOW);
      digitalWrite(ledLuzAlto, LOW);
    } else if (luminosity >= 20 && luminosity < 60) {
      digitalWrite(ledLuzBajo, LOW);
      digitalWrite(ledLuzMedio, HIGH);
      digitalWrite(ledLuzAlto, LOW);
    } else if (luminosity >= 60 && luminosity <= 100) {
      digitalWrite(ledLuzBajo, LOW);
      digitalWrite(ledLuzMedio, LOW);
  ...
  delay(8000); // Enviar datos cada 5 segundos
